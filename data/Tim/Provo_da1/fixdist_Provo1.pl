...
print FH "subject\torder\titem\tcondition\tline\twordnum\tword\tStart\tEnd\tUndersweep\tLine1stFix\tLaunch\tAltLaunch\tLand\tAltLand\tSacLen\tAltSacLen\tSacDur\tAltSacDur\tFF\tAltFF\tGaze\tGazeOutlier\tAltGaze\tAltGazeOutlier\tSP\tAltSP\tRO\tAltRO\tRI\tAltRI\tTotalTime\n";#print header
while ($filename = <LISTFILE>)
  {
	...
	for($l = 0; $l < $maxline + 1; $l++)#cycle through the lines of text for an item one at a time
	{
		...
		for($p = 1; $p < $numword + 1; $p++)#then we cycle through the words on the line one at a time to calculate the eye measures for each word
		{
			...
			for($x = 12; $x < $numel; $x = $x + 4)#this cycles through the fixations on the trial (skipping the first) looking for ones on the target word we got in the loop above
			{
				...
				if($dataline[$x+1] > $maxY)
				{
					...
					if(($dataline[$x+4]<$startx) and ($x+4 < $numel-1) and ($dataline[$x+5] > -1))
					{
			...
			print FH "\t$launch\t$Altlaunch\t$land\t$Altland\t$saccadelength\t$AltSaccadelength\t$SacDur\t$AltSacDur\t$FF\t$AltFF\t$Gaze\t$gazeout\t$AltGaze\t$altgazeout\t$SP\t$AltSP\t$RI\t$AltRI\t$RO\t$AltRO\t$TT\n";
		}#end of p loop
	}#end of l loop
  }
close INDATA;
