...
print FH "subject\torder\titem\tcondition\tline\twordnum\tword\tStart\tEnd\tUndersweep\tLine1stFix\tLaunch\tAltLaunch\tLand\tAltLand\tSacLen\tAltSacLen\tSacDur\tAltSacDur\tFF\tAltFF\tGaze\tGazeOutlier\tAltGaze\tAltGazeOutlier\tSP\tAltSP\tRO\tAltRO\tRI\tAltRI\tTotalTime\tblinkbad\n";#print header
while ($filename = <LISTFILE>)
  {
	...
	for($l = 0; $l < $maxline + 1; $l++)#cycle through the lines of text for an item one at a time
	{
		...
		for($p = 1; $p < $numword + 1; $p++)#then we cycle through the words on the line one at a time to calculate the eye measures for each word
		{
			...
			for($x = 13; $x < $numel; $x = $x + 5)#this cycles through the fixations on the trial (skipping the first) looking for ones on the target word we got in the loop above
			{
				if((($dataline[$x+3]-$dataline[$x+2])>80) and ($dataline[$x+1] > -1))#excludes short and off passage fixations from entering the calculations
				{
				$firstlinefix = 0;
				...
				if($dataline[$x+1] > $maxY)
				{
					...
					if(($dataline[$x+5]<$startx) and ($x+5 < $numel-1) and ($dataline[$x+6] > -1))
					{
				...
				if(($dataline[$x] > $endx)and($dataline[$x+1]==$l)and($launch eq "na"))
				{
					if($x==8){
						...
						$land = $dataline[$x] - $startx;#space counts as first position!
			...
			print FH "\t$launch\t$Altlaunch\t$land\t$Altland\t$saccadelength\t$AltSaccadelength\t$SacDur\t$AltSacDur\t$FF\t$AltFF\t$Gaze\t$gazeout\t$AltGaze\t$altgazeout\t$SP\t$AltSP\t$RI\t$AltRI\t$RO\t$AltRO\t$TT\t$blinkbad\n";
		}#end of p loop
	}#end of l loop
  }
close INDATA;
